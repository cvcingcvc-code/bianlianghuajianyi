...
- 模拟账户内存保存，重启清空；不是完整成交撮合/清算引擎。2026-09-22 已补齐风控 SL/TP 的独立模拟触发平仓，规则和验收见下文。
...
- 保持信号在 N 收盘产生、订单在 N+1 开盘成交。先处理应在开盘成交的排队订单，再检查仍持有的 LONG 仓位；开仓成交当根即可触发保护。
- `low <= stopLossPrice` 止损，`high >= takeProfitPrice` 止盈。双触发固定 `STOP_LOSS FIRST`，记录 `intrabarAmbiguous: true` 和 `reason: STOP_LOSS_AMBIGUOUS_BAR`。
- 止损遇 `open <= SL`、止盈遇 `open >= TP` 时使用开盘价，否则使用触发价，再扣现有滑点 0.0002 和手续费 0.0004。
- 成交后删除仓位和待平仓订单、结算钱包与已实现盈亏、保存 fill 并发出事件。既有 Pipeline 将 CLOSE 转发 RiskBridge，亏损进入熔断统计。
- 同根已排队的策略 CLOSE 在开盘成交后，不再按该根后续 high/low 产生保护性成交。保护性退出后的空仓 CLOSE 被拒绝，不重复结算。
- fill 的 `pnl` 沿用单次结算口径：平仓毛损益减平仓手续费；开仓手续费已在 OPEN 时扣除，不重复扣费。当日已实现盈亏累计当日成交现金变动，dailyPnl 基于日初权益，包含持仓浮盈亏变化。
- 跳空成交时间为 `BAR_OPEN`；非跳空以 K 线收盘为检测时间，`timestampBasis: BAR_CLOSE_DETECTION`，不声称知道真实盘中成交时间。
- 新增 12 项算术/集成测试，覆盖 SL、TP、不触发、双触发、双向跳空、重复平仓、成本对账、跨日盈亏、入场时序和五次亏损熔断。测试价格仅为执行算术夹具，不是市场数据或研究结果。
- `npm run test:v1`：27/27 全部通过，包括原有官方数据 next-bar 入场测试；`npm test`：242 项、229 通过、13 项既有数据/校验失败，失败集合与基线一致，无新增回归。
- 本次未修改 UI、冻结策略、历史研究结果、舆情或 Live；未执行新的策略收益验证。资金费率、真实盘中路径及会话持久化仍不在本轮范围内。
